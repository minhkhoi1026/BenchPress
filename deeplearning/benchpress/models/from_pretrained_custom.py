...
class PreTrainedModel(object):
    ...
    def FromID(cls, name: str = "base_opencl") -> "PreTrainedModel":
        if name not in PRETRAINED_MODELS:
            raise ValueError(
                "Pre-trained model {} does not exist. Available models: {}".format(
                    name, ", ".join([x for x in PRETRAINED_MODELS.keys()])
                )
            )
        ...
        if not tokenizer_path.exists():
            gdown.download(
                "https://drive.google.com/uc?id={}".format(
                    PRETRAINED_MODELS[name]["tokenizer"]
                ),
                str(tokenizer_path),
            )
        ...
        model_config = pbutil.FromFile(
            config_path, benchpress_pb2.Instance()
        ).language_model
    ...
    def __init__(
        ...
        if environment.WORLD_RANK == 0:
            if not self.language_model.corpus.tokenizer_path.exists():
                shutil.copyfile(
                    tokenizer_path, self.language_model.corpus.tokenizer_path
                )
            if not (
                self.language_model.cache.path / "checkpoints" / "backup_tokenizer.pkl"
            ).exists():
                shutil.copyfile(
                    tokenizer_path,
                    self.language_model.cache.path
                    / "checkpoints"
                    / "backup_tokenizer.pkl",
                )
            if not (
                self.language_model.cache.path / "checkpoints" / "model-0.pt"
            ).exists():
                shutil.copyfile(
                    checkpoint,
                    self.language_model.cache.path / "checkpoints" / "model-0.pt",
            ...
            if not (
                self.language_model.cache.path / "checkpoints" / "checkpoint.meta"
            ).exists():
                with open(
                    self.language_model.cache.path / "checkpoints" / "checkpoint.meta",
                    "w",
        ...
        if pytorch.num_gpus == 0:
            l.logger().warn(
                "No GPUs detected. This process is going to be *very* slow on the CPU."
            )
        return
    ...
    def Sample(
        ...
        if "[START]" in prompt or "[END]" in prompt:
            l.logger().error(
                "Do not add [START] and [END] manually. They will be added automatically by the tokenizer."
            )
            return ""
        prompt = "[START]" + prompt + "[END]"
        test_sampler = self.getTestSampler(
            prompt,
            batch_size,
            temperature,
        ...
        self.language_model.Sample(
            test_sampler, obs, num_batches=num_batches, seed=seed
        )
    ...
    def getTestSampler(
        ...
        sampler = samplers.Sampler(mock_config, sample_db_name=sample_db_name)
